...
class ApoliceSerializer(serializers.ModelSerializer):
    ...
    modalidade_nome = serializers.CharField(
        source="cotacao.modalidade.nome", read_only=True
    )
    segurado_nome = serializers.CharField(
        source="cotacao.segurado.nome", read_only=True, allow_null=True
    )
    edital = serializers.CharField(
        source="cotacao.edital", read_only=True, allow_null=True
    )
    data_inicio = serializers.DateField(
        source="cotacao.data_inicio", read_only=True, allow_null=True
    )
    data_final = serializers.DateField(
        source="cotacao.data_final", read_only=True, allow_null=True
    )
    prazo_dias = serializers.IntegerField(
        source="cotacao.prazo_dias", read_only=True, allow_null=True
    )
    importancia_segurada = serializers.DecimalField(
        source="cotacao.importancia_segurada",
        max_digits=14,
        decimal_places=2,
        read_only=True,
        allow_null=True,
